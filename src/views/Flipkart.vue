  ...
  methods: {
    ...
    async uploadFile() {
      ...
      for (const base of this.baseUrls) {
        try {
          const res = await axios.post(`${base}/upload-flipkart`, formData, {
            headers: { "Content-Type": "multipart/form-data" },
          });

          this.results = res.data.results;
          uploaded = true;
          break; // ✅ Stop trying if upload succeeds
        } catch (err) {
          console.warn(`Failed to upload to ${base}/upload-flipkart, trying next...`, err.message);
        }
      }
